alphafrontier.2d <- function(xobs, yobs, type = "output", alpha = 0.95,
                             add = FALSE, confidence = FALSE, shade = FALSE, ...) {
    # verification
    ...
    if (type == "output") {
      ...
      if (confidence) {
        linspace.x <- as.matrix(c(seq(min(xobs), max(xobs), diff(range(xobs)) / 1000),
                                  bound[2]))
        linspace.y <- as.matrix(c(seq(min(yobs), max(yobs), diff(range(yobs)) / 1000), 
                                  max(yobs)))
